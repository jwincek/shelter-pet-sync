...
| **Plugin dir/slug/text-domain → `shelterkit-pets`** | Optional rename for consistency; needs a reactivate. | step 3 (optional) |
...
The text domain is now `shelterkit-pets`; matching the folder avoids a
...
mv PLUGIN_DIR shelterkit-pets
wp plugin activate shelterkit-pets
```
...
From here, `PLUGIN_DIR` is `shelterkit-pets` if you renamed it.

## 4. Migrate the post type (`pet` → `vcps_pet`)

```bash
wp eval-file wp-content/plugins/shelterkit-pets/migration-scripts/2026-06-23-rename-cpt-pet-to-vcps_pet.php
```
...
```bash
wp eval-file wp-content/plugins/shelterkit-pets/migration-scripts/2026-06-23-purge-pii-from-api-snapshots.php
```
...
wp maintenance-mode activate
cd wp-content/plugins
rm -rf shelterkit-pets   # or the renamed/checked-out tree
