...
// === Sort options ===
$sort_options = array(
	''          => __( 'Newest First', 'shelterkit-pets' ),
	'name-asc'  => __( 'Name A–Z', 'shelterkit-pets' ),
	'name-desc' => __( 'Name Z–A', 'shelterkit-pets' ),
	'age-asc'   => __( 'Youngest First', 'shelterkit-pets' ),
	'age-desc'  => __( 'Oldest First', 'shelterkit-pets' ),
	'random'    => __( 'Random', 'shelterkit-pets' ),
);
...
$filter_config = array(
	...
	'sex'    => array(
		...
		'all'   => __( 'Any Sex', 'shelterkit-pets' ),
		'show'  => $filter_sex,
	),
	'size'   => array(
...
<div class="pet-listing-grid__inner" <?php echo $inner_attributes; /* phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- built from wp_interactivity_data_wp_context() plus static directives. */ ?>>
		...
		<div class="pet-listing-grid__toolbar">
			...
			<div class="pet-listing-grid__toolbar-row">
				<?php if ( $show_search ) : ?>
					<div class="pet-listing-grid__search">
						<label for="pet-search" class="screen-reader-text">
							<?php esc_html_e( 'Search pets', 'shelterkit-pets' ); ?>
						</label>
						<div class="pet-listing-grid__search-wrapper">
							<input
								...
								placeholder="<?php esc_attr_e( 'Search by name or breed...', 'shelterkit-pets' ); ?>"
							...
							<button
								...
								aria-label="<?php esc_attr_e( 'Clear search', 'shelterkit-pets' ); ?>"
							>
							...
							<button
								...
								aria-label="<?php esc_attr_e( 'Search', 'shelterkit-pets' ); ?>"
							>
								<?php
				...
				<?php if ( $show_filters ) : ?>
					...
					>
						...
						<span><?php esc_html_e( 'Filters', 'shelterkit-pets' ); ?></span>
						<span
				...
				<div class="pet-listing-grid__sort">
					<label class="screen-reader-text" for="pet-sort">
						<?php esc_html_e( 'Sort by', 'shelterkit-pets' ); ?>
					</label>
					<select
						id="pet-sort"
			...
			<?php if ( $show_filters ) : ?>
				...
				<div class="pet-listing-grid__filter-drawer-inner">
					<div class="pet-listing-grid__filters" role="group" aria-label="<?php esc_attr_e( 'Filter pets', 'shelterkit-pets' ); ?>">
						<?php
						...
						>
							<?php esc_html_e( 'Clear All', 'shelterkit-pets' ); ?>
						</button>
					</div>
						...
						<details class="pet-listing-grid__compat-details" open data-has-active="<?php echo $active_compat_count > 0 ? '1' : '0'; ?>">
							<summary class="pet-listing-grid__compat-summary">
								...
								printf(
									...
									esc_html__( 'More Filters (%d)', 'shelterkit-pets' ),
							...
							<div class="pet-listing-grid__compat-filters pet-listing-grid__compat-filters--<?php echo esc_attr( $compat_style ); ?>" role="group" aria-label="<?php esc_attr_e( 'Compatibility filters', 'shelterkit-pets' ); ?>">
	...
	>
		<span class="pet-listing-grid__loading-text" hidden data-wp-bind--hidden="!state.isNavigating">
			<?php esc_html_e( 'Updating results…', 'shelterkit-pets' ); ?>
		</span>
	</div>
	...
	<div class="pet-listing-grid__results-bar">
		...
		<div data-wp-interactive="petsync/grid">
			...
			>
				<?php esc_html_e( "\u{2665} Favorites", 'shelterkit-pets' ); ?>
			</button>
		</div>
			...
			<div class="pet-listing-grid__results-info" role="status" aria-live="polite">
				...
				if ( $search_query ) {
					printf(
						...
						esc_html( _n( '%1$d result for &ldquo;%2$s&rdquo;', '%1$d results for &ldquo;%2$s&rdquo;', $total, 'shelterkit-pets' ) ),
						(int) $total,
						esc_html( $search_query )
					);
				} else {
					printf(
						esc_html( /* translators: %d: number of pets */ _n( '%d pet', '%d pets', $total, 'shelterkit-pets' ) ),
						(int) $total
					);
				}
				?>
	...
	$compat_labels = array(
		'goodWithDogs'   => __( 'Good with dogs', 'shelterkit-pets' ),
		'goodWithCats'   => __( 'Good with cats', 'shelterkit-pets' ),
		'goodWithKids'   => __( 'Good with kids', 'shelterkit-pets' ),
		'shotsCurrent'   => __( 'Shots current', 'shelterkit-pets' ),
		'spayedNeutered' => __( 'Spayed/Neutered', 'shelterkit-pets' ),
		'housebroken'    => __( 'Housebroken', 'shelterkit-pets' ),
		'specialNeeds'   => __( 'Special needs', 'shelterkit-pets' ),
	);
	...
	if ( $search_query ) {
		$active_chips[] = array(
			'type'  => 'search',
			'key'   => 'search',
			'label' => sprintf( /* translators: %s: search query */ __( 'Search: "%s"', 'shelterkit-pets' ), $search_query ),
		);
	}

	if ( ! empty( $active_chips ) ) :
		?>
		<div class="pet-listing-grid__active-filters" role="list" aria-label="<?php esc_attr_e( 'Active filters', 'shelterkit-pets' ); ?>">
			<?php foreach ( $active_chips as $chip ) : ?>
				<button
					...
					role="listitem"
					aria-label="<?php echo esc_attr( sprintf( /* translators: %s: active filter label */ __( 'Remove filter: %s', 'shelterkit-pets' ), $chip['label'] ) ); ?>"
				>
			...
			<?php if ( count( $active_chips ) > 1 ) : ?>
				<button
					...
					data-wp-on--click="actions.resetFilters"
					aria-label="<?php esc_attr_e( 'Clear all filters', 'shelterkit-pets' ); ?>"
				>
					<?php esc_html_e( 'Clear all', 'shelterkit-pets' ); ?>
				</button>
			<?php endif; ?>
	...
	<ul class="pet-listing-grid__grid" role="list">
		...
		foreach ( $pets as $index => $pet ) :
			...
			switch ( $badge_type ) {
				...
				case 'new':
					if ( $pet['is_new'] ?? false ) {
						$badge_text  = __( 'New!', 'shelterkit-pets' );
					...
					<div class="pet-listing-grid__card-content">
								...
								<?php if ( $pet['is_bonded_pair'] ?? false ) : ?>
									<span class="pet-listing-grid__badge-popover-anchor">
										...
										>
											...
											<?php esc_html_e( 'Bonded Pair', 'shelterkit-pets' ); ?>
										</button>
										<div
										...
										>
											...
											<p class="pet-listing-grid__bonded-popover-label"><?php esc_html_e( 'Must adopt together with:', 'shelterkit-pets' ); ?></p>
						...
						<div class="pet-listing-grid__card-actions">
							<button
								...
								aria-label="<?php esc_attr_e( 'Toggle favorite', 'shelterkit-pets' ); ?>"
							>
								<?php
	...
	>
		<p><?php esc_html_e( 'No pets match your filters.', 'shelterkit-pets' ); ?></p>
		<button type="button" class="pet-listing-grid__empty-reset" data-wp-on--click="actions.resetFilters">
			<?php esc_html_e( 'Show all pets', 'shelterkit-pets' ); ?>
		</button>
	</div>
	...
	if ( $total_pages > 1 ) :
		...
		sort( $pages_to_show );
		?>
		<nav class="pet-listing-grid__pagination" aria-label="<?php esc_attr_e( 'Pet listing pages', 'shelterkit-pets' ); ?>">
			<?php // Prev button ?>
			<?php if ( $paged > 1 ) : ?>
				<a
					...
					aria-label="<?php esc_attr_e( 'Previous page', 'shelterkit-pets' ); ?>"
				>
			...
			<?php if ( $paged < $total_pages ) : ?>
				<a
					...
					data-wp-on--click="actions.navigateToPage"
					data-wp-on--mouseenter="actions.prefetchPage"
					aria-label="<?php esc_attr_e( 'Next page', 'shelterkit-pets' ); ?>"
				>
