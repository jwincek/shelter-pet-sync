...
$title              = $attributes['title'] ?? __( 'Meet Our Pets', 'shelterkit-pets' );
...
$cta_text           = $attributes['ctaText'] ?? __( 'Find Your New Best Friend', 'shelterkit-pets' );
$show_cta           = $attributes['showCta'] ?? false;
$link_to_archive    = $attributes['linkToArchive'] ?? true;
$archive_link_text  = $attributes['archiveLinkText'] ?? __( 'View All Pets', 'shelterkit-pets' );
					...
					>
						...
						<div class="pet-slider__hero-overlay-info">
							...
							<a 
								...
								data-wp-bind--href="state.currentPetUrl"
							>
								<?php esc_html_e( 'Meet Me', 'shelterkit-pets' ); ?>
								<?php
								Petsync_Icons::render(
									'arrow-right',
									array(
					...
					<div class="pet-slider__hero-thumbs">
						<?php foreach ( $pets as $index => $pet ) : ?>
							<button
								...
								aria-label="<?php echo esc_attr( sprintf( /* translators: %s: pet name */ __( 'View %s', 'shelterkit-pets' ), $pet['name'] ) ); ?>"
							>
								<img 
	...
	<?php else : ?>
		...
		<?php if ( $show_navigation && count( $pets ) > 1 ) : ?>
			<button
				...
				aria-label="<?php esc_attr_e( 'Previous pet', 'shelterkit-pets' ); ?>"
			>
				<?php
				Petsync_Icons::render(
					'chevron-left',
			...
			<button
				...
				aria-label="<?php esc_attr_e( 'Next pet', 'shelterkit-pets' ); ?>"
			>
				<?php
				Petsync_Icons::render(
					'chevron-right',
		...
		<?php if ( $show_dots && count( $pets ) > 1 ) : ?>
			<div
				...
				role="tablist" 
				aria-label="<?php esc_attr_e( 'Slide navigation', 'shelterkit-pets' ); ?>"
